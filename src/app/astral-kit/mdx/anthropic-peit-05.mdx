...
Modify the ""best basketball player"" prompt in the highlighted template box and use the ""speaking for Claude"" technique (putting words after ""Assistant:"") to <Mark>compell Claude to make a detailed argument that the best basketball player of all time is **Stephen Curry**</Mark>. Claude's response will have a green check mark if your prompt produces the right answer.
