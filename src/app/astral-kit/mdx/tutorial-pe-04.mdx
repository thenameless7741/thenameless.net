...
<Playground
  user={`User: I will tell you the name of an animal. Please respond with the noise that animal makes. {{ANIMAL}}`}
  assistant={`Moo.`}
...
<Playground
  user={`User: Yo Claude. {{EMAIL}} <----- Make this email more polite but don't change anything else about it.`}
...
<Playground
  user={`User: Yo Claude. <email>{{EMAIL}}</email> <----- Make this email more polite but don't change anything else about it.`}
...
<Playground
  user={`User: Below is a list of sentences. Tell me the second item on the list.\n
...
<Playground
  user={`User: Write a haiku about {{TOPIC}}.`}
...
<Playground
  user={`User: Hia its me i have a q about dogs jkaerjv <question>{{QUESTION}}</question> jklmvca tx it help me muhch much atx fst fst answer short short tx`}
