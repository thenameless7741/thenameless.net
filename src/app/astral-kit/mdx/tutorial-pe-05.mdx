...
<Playground
  user={`User: Please write a haiku about {{ANIMAL}}. Put it in <haiku> tags.`}
  assistant={`<haiku>
...
<Playground
  user={`User: Please write a haiku about {{ANIMAL}}. Use JSON format with the keys as "first_line", "second_line", and "third_line".
Assistant: {`}
...
<Playground
  user={`User: Hey Claude. Here is an email: <email>{{EMAIL}}</email>. Make this email more {{ADJECTIVE}}. Write the new version in <{{ADJECTIVE}}_email> XML tags.
...
<Playground
  user={`User: Who is the best basketball player of all time? Please choose one specific player.
...
<Playground
  user={`User: Please write two haikus about {{ANIMAL}}. Put each in <haiku> tags.
Assistant: <haiku>`}
  assistant={`Feline grace and poise,
...
</haiku>`} labels={{ user: 'Prompt Template', assistant: "Claude's Response" }}
input={{ ANIMAL: `cats` }}
/>
...
<Playground
  user={`User: Please write two haikus, one about {{ANIMAL1}} and one about {{ANIMAL2}}. Put each in <haiku> tags.`}
