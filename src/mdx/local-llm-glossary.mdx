...
If you prefer to read this note from beginning to end, I recommend checking out [the original version](https://osanseviero.github.io/hackerllama/blog/posts/hitchhiker_guide) authored by [Omar Sanseviero](https://twitter.com/osanseviero), as it is structured with progressive disclosure.
...
**CFG (Classifier-Free Guidance)**: a technique to encourage prompt-adherence in text generation. It enables features such as guidance_scale and negative prompt in text-generation-webui. It emerged in text-to-image generation and is now available for text generation. Reference: [CFG paper](https://arxiv.org/abs/2306.17806).
...
**DynaTemp (Dynamic Temperature)**: An alternate sampling method to Top P and Top K that dynamically adjusts the temperature when sampling different tokens. DynaTemp first analyzes the probabilities assigned to each token by the language model. For tokens with relatively low probabilities (indicating more challenging choices), it applies a higher temperature, increasing the likelihood of selecting less probable but potentially more interesting or creative options. For tokens with high probabilities (indicating confident predictions), it employs a lower temperature, reducing the influence of random noise and ensuring more consistent, coherent output. References: [llama.cpp's FR](https://github.com/ggerganov/llama.cpp/issues/3483), [AdapT paper](https://arxiv.org/abs/2309.02772).
...
**Sampling**: the process of randomly selecting tokens (words, phrases, etc.) from a distribution of probabilities generated by a language model.
