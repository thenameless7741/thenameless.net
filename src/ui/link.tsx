...
const Component = ({
  ...
  if (!internal && showIcon) {
    if (exps.arxiv.test(href)) {
      Icon = ArxivLogo;
    } else if (exps.anthropic.test(href)) {
      Icon = AnthropicLogo;
...
// ^(https?://)?([\w-]+\.)?anthropic\.com$
const exps = {
  arxiv: /^https?:\/\/(www\.)?arxiv\.org/,
  anthropic: /^https?:\/\/([\w-]+\.)?anthropic\.com/,
...
const AnthropicLogo = ({ color = 'currentColor' }: { color?: string }) => {
  return (
    <svg
      className={[s.icon, s.anthropic].join(' ')}
      viewBox="0 0 320 320"
      xmlns="http://www.w3.org/2000/svg"
    >
      <path
        d="M230.721 47H181.765L271.043 272.75H320L230.721 47ZM89.2786 47L0 272.75H49.9227L68.1802 225.342H161.583L179.84 272.75H229.763L140.484 47H89.2786ZM84.33 183.416L114.881 104.079L145.433 183.416H84.33Z"
        fill={color}
      />
    </svg>
  );
